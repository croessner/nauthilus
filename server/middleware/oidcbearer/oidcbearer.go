...
// HasExactAudience reports whether the normalized audience set contains only the expected resource.
func HasExactAudience(claims jwt.MapClaims, expected string) bool {
	if expected == "" {
		return false
	}
