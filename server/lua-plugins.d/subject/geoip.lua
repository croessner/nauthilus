...
-- Shared helpers for subject source and action
...
local function write_rt_geoip(rt, guid, current_iso_code, iso_codes_seen, status, is_subject)
    rt.subject_geoippolicyd = is_subject or false
    rt.geoip_info = {
        guid = guid or "",
...
function nauthilus_call_subject(request)
    ...
    -- Use a dedicated namespace for subject cache to avoid mixing with action cache
    ...
    if response.err == nil then
        ...
        return nauthilus_builtin.SUBJECT_ACCEPT, nauthilus_builtin.SUBJECT_RESULT_FAIL
    end
