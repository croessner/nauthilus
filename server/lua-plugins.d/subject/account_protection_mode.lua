...
-- The subject source turns long-window account metrics and account-centric attack flags
-- into policy-visible Lua subject-source results plus request-local policy facts.
--
-- Keys used:
...
function nauthilus_call_subject(request)
    ...
    if under then
        ...
        if type(rt) == "table" then
            ...
            rt.subject_account_protection_mode = true
            nauthilus_context.context_set("rt", rt)
        end

        -- Decide subject result: If authentication failed, we either reject (enforcement) or allow (dry-run)
