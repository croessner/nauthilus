...
func TestBuiltinStandardAuthEffectsResolveCanonicalIdentitiesAndTypedParameters(t *testing.T) {
	...
	standardAuth, ok := target.LookupPolicySet(mustCatalogSetID(t, registry.BuiltinStandardAuthPolicySet))
	if !ok {
		t.Fatal("builtin standard_auth policy set missing")
	}

	var bruteForceEffects []registry.EffectUse

	for _, rule := range standardAuth.Rules() {
		if rule.Name() == "standard_brute_force_deny" {
			bruteForceEffects = rule.Effects()

			break
		}
	}

	if len(bruteForceEffects) != 1 {
		t.Fatalf("standard brute-force effects = %d, want 1", len(bruteForceEffects))
	}

	assertStandardAuthEffects(t, target, bruteForceEffects)
	assertStandardAuthProviderOrder(t, target)
}

func TestBuiltinStandardAuthEffectsUseExactCanonicalTargetAllowlists(t *testing.T) {
	...
	want := map[string][]string{
		policy.EffectBruteForceUpdate: {"authn/authenticate", "authn/lookup_identity"},
	}
	...
	for _, effect := range contribution.Effects() {
		...
		if !reflect.DeepEqual(targets, want[effect.ID()]) {
			t.Fatalf("effect %s targets = %v, want %v", effect.ID(), targets, want[effect.ID()])
		}
	}
}

func TestBuiltinStandardAuthEffectsUseExactCanonicalOwnersAndClasses(t *testing.T) {
	contribution, err := registry.NewBuiltinTargetContributor(catalogAcceptanceCapability{}).Contribute(context.Background())
	if err != nil {
		t.Fatalf("Contribute() error = %v", err)
	}
	...
	for _, effect := range contribution.Effects() {
		expected, exists := want[effect.ID()]
		if !exists {
			t.Fatalf("unexpected builtin effect %q", effect.ID())
		}
...
// assertStandardAuthEffects resolves builtin rule effects through the canonical catalog.
func assertStandardAuthEffects(
	t *testing.T,
	...
	for _, use := range uses {
		effect, exists := target.LookupEffect(use.ID())
		if !exists {
			t.Fatalf("standard_auth effect %q is not registered", use.ID())
		}

		if !effect.IsBuiltin() {
			t.Fatalf("standard_auth effect %q has a configurable execution class", use.ID())
		}

		if err := effect.ValidateUse(use); err != nil {
			t.Fatalf("ValidateUse(%q) error = %v", use.ID(), err)
		}

		provider, exists := target.LookupProvider(effect.Provider())
		if !exists || !provider.IsBuiltin() {
			t.Fatalf("standard_auth effect %q has no immutable builtin provider", use.ID())
		}
	}
}
