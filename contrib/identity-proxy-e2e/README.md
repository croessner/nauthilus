...
ok oidc-heavy-webauthn-reference-cookie-bounded
ok oidc-heavy-webauthn-registration-both-edges
ok recovery-code-generation
ok mfa-self-service-missing-step-up-rejected
ok mfa-self-service-recovery-regeneration-step-up
ok mfa-self-service-totp-delete-step-up
ok mfa-self-service-webauthn-delete-step-up
...
ok oidc-required-enrollment-webauthn-login
...
ok oidc-master-user-without-mfa-require-mfa-rejected
ok oidc-delayed-response-master-user-without-mfa-require-mfa-rejected
...
ok oidc-recovery-code-login
ok oidc-delayed-response-recovery-invalid-code
ok oidc-delayed-response-recovery-code-login
ok oidc-delayed-response-recovery-wrong-password-rejected
ok oidc-recovery-code-reuse-rejected
ok oidc-master-user-recovery-invalid-code
ok oidc-master-user-recovery-code-login
ok oidc-delayed-response-master-user-recovery-invalid-code
ok oidc-delayed-response-master-user-recovery-code-login
ok oidc-delayed-response-master-user-recovery-wrong-password-rejected
ok oidc-master-user-recovery-code-reuse-rejected
ok multi-edge-oidc-continuity
ok multi-edge-webauthn-continuity
ok oidc-heavy-webauthn-login-both-edges
ok saml-sso-login
...
| `oidc-heavy-webauthn-registration-both-edges`         | Browser MFA continuity        | A deliberately cookie-heavy OIDC flow starts on edge-a and completes WebAuthn registration on edge-b.                                                         | Registration succeeds without growing the primary cookie with the ceremony reference.                                                                                                |
| `oidc-heavy-webauthn-reference-cookie-bounded`        | Browser cookie boundary       | The heavy OIDC registration pauses before WebAuthn finish and inspects both encrypted cookies.                                                                 | The primary cookie remains present while the dedicated Secure, HttpOnly, SameSite=Lax, Path=/ reference cookie stays below 512 bytes.                                                |
...
| `mfa-self-service-missing-step-up-rejected`            | Browser MFA self-service      | A first-factor-only session tries to mutate recovery, TOTP, and WebAuthn self-service state.                                                                  | Sensitive self-service mutations render `Recent MFA verification required` before reaching authority-backed mutations.                                                                |
| `mfa-self-service-recovery-regeneration-step-up`       | Browser MFA self-service      | A browser session with fresh MFA step-up regenerates recovery codes through the edge.                                                                          | Recovery-code regeneration succeeds only after fresh MFA assurance and returns one-time plaintext codes.                                                                               |
| `mfa-self-service-totp-delete-step-up`                 | Browser MFA self-service      | A browser session with fresh MFA step-up deletes a TOTP factor through the edge.                                                                               | TOTP deletion succeeds after fresh MFA assurance and redirects back to the 2FA overview.                                                                                              |
| `mfa-self-service-webauthn-delete-step-up`             | Browser MFA self-service      | A browser session with fresh MFA step-up deletes a WebAuthn device through the edge.                                                                           | WebAuthn deletion succeeds after fresh MFA assurance and redirects back to the WebAuthn device list.                                                                                  |
...
| `oidc-recovery-code-login`                            | Browser MFA                   | Positive baseline for a registered recovery-code factor after enrollment.                                                                                     | A generated recovery code can complete an OIDC login for the user.                                                                                                                    |
| `oidc-delayed-response-recovery-invalid-code`         | Browser MFA negative          | Recovery-code guessing is attempted while the client has `delayed_response` enabled.                                                                          | The user stays on the recovery-code page and sees `Invalid recovery code`.                                                                                                            |
| `oidc-delayed-response-recovery-code-login`           | Browser MFA                   | Positive baseline for recovery-code login when the client has `delayed_response` enabled.                                                                     | A correct first factor and generated recovery code complete the OIDC flow with `delayed_response` enabled.                                                                            |
| `oidc-delayed-response-recovery-wrong-password-rejected` | Browser MFA negative       | A wrong password is hidden until after recovery-code MFA when `delayed_response` is enabled.                                                                  | Completing recovery-code MFA after a wrong first factor returns to the login page with a generic authentication error.                                                                 |
...
| `oidc-required-enrollment-webauthn-login`             | Browser MFA                   | Required enrollment rules list TOTP and recovery codes while WebAuthn remains an explicitly supported level-3 challenge.                                      | WebAuthn satisfies the level-2 step-up without weakening or reinterpreting the required enrollment set.                                                                               |
...
| `oidc-master-user-recovery-invalid-code`              | Browser MFA negative          | Recovery-code guessing is attempted for a formatted Master-User login.                                                                                        | The Master-User MFA challenge stays on the recovery-code page and rejects the invalid code.                                                                                           |
| `oidc-master-user-recovery-code-login`                | Browser MFA                   | A Master-User login completes recovery-code MFA for a target account.                                                                                         | The recovery code belongs to the Master-User, while the issued ID token contains the target account in `preferred_username`.                                                          |
| `oidc-delayed-response-master-user-recovery-invalid-code` | Browser MFA negative       | Recovery-code guessing is attempted for a formatted Master-User login with `delayed_response` enabled.                                                        | The Master-User MFA challenge stays on the recovery-code page and rejects the invalid code.                                                                                           |
| `oidc-delayed-response-master-user-recovery-code-login` | Browser MFA                 | A Master-User recovery-code login works when the client has `delayed_response` enabled.                                                                       | The Master-User's recovery code completes the flow and the issued ID token contains the target account.                                                                                |
| `oidc-delayed-response-master-user-recovery-wrong-password-rejected` | Browser MFA negative | A wrong Master-User password is hidden until after the Master's recovery code when `delayed_response` is enabled.                                               | Completing recovery-code MFA after a wrong Master-User first factor returns to the login page with a generic authentication error.                                                     |
| `oidc-master-user-without-mfa-login`                  | Browser MFA                   | A Master-User account with no registered factors can still authenticate a target account for clients without mandatory MFA.                                    | The flow completes without an MFA challenge and the issued ID token contains the target account, not the formatted Master-User login string.                                           |
| `oidc-master-user-without-mfa-require-mfa-rejected`   | Browser MFA negative          | A Master-User account with no registered factors attempts a `require_mfa` authorization-code flow.                                                            | The flow is held at MFA selection and no authorization code is issued without recent MFA assurance.                                                                                    |
| `oidc-delayed-response-master-user-without-mfa-require-mfa-rejected` | Browser MFA negative | A Master-User account with no registered factors attempts a `delayed_response` `require_mfa` authorization-code flow.                                          | The flow is held at MFA selection and no authorization code is issued without recent MFA assurance.                                                                                    |
...
| `oidc-master-user-recovery-code-reuse-rejected`       | Browser MFA negative          | A consumed Master-User recovery code is replayed as a second factor for a formatted login.                                                                    | A consumed Master-User recovery code cannot be used a second time.                                                                                                                    |
...
| `oidc-heavy-webauthn-login-both-edges`                | Browser continuity            | A deliberately cookie-heavy OIDC login starts on edge-a and completes its WebAuthn ceremony on edge-b.                                                        | The OIDC callback succeeds while both edges consume the dedicated ceremony-reference cookie contract.                                                                                |
...
| `saml-slo-missing-payload-rejected`                   | SAML SLO negative             | An attacker probes `/saml/slo` without either `SAMLRequest` or `SAMLResponse`.                                                                                | The canonical checkpoint rejects the unbound continuation with HTTP 409 before the SLO handler runs.                                                                                 |
| `saml-slo-ambiguous-payload-rejected`                 | SAML SLO negative             | Payload parameter pollution sends both `SAMLRequest` and `SAMLResponse` in one SLO message.                                                                  | The canonical checkpoint rejects the ambiguous unbound continuation with HTTP 409 before the SLO handler runs.                                                                       |
| `saml-slo-duplicate-request-rejected`                 | SAML SLO negative             | Duplicate `SAMLRequest` parameters attempt to exploit parser disagreement.                                                                                    | The canonical checkpoint rejects the duplicated unbound continuation with HTTP 409 before the SLO handler runs.                                                                      |
