...
func TestListAccountsRequestToDTOMapsEveryField(t *testing.T) {
	...
	got := ListAccountsRequestToDTO(DTOToListAccountsRequest(expected))
	if !reflect.DeepEqual(got, expected) {
		t.Fatalf("unexpected DTO mapping:\n got: %+v\nwant: %+v", got, expected)
	}
}
...
func TestLookupIdentityRequestToDTOMapsEveryField(t *testing.T) {
	...
	got := LookupIdentityRequestToDTO(DTOToLookupIdentityRequest(expected))
	if !reflect.DeepEqual(got, expected) {
		t.Fatalf("unexpected DTO mapping:\n got: %+v\nwant: %+v", got, expected)
	}
}
...
func newPopulatedAuthRequest() *authv1.AuthRequest {
	return DTOToAuthRequest(expectedAuthDTO())
}
