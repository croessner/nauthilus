...
//
// Some backend tests swap the global client via `rediscli.NewTestClient(db)`.
// The hard-fail backend Redis seam requires a non-nil default client.
...
func (globalTestClient) GetReadHandles() []redis.UniversalClient {
	return rediscli.GetClient().GetReadHandles()
}
