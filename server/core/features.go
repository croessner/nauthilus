...
func (a *AuthState) featureLua(ctx *gin.Context) (triggered bool, abortFeatures bool, err error) {
	...
	if !(a.ClientIP == global.Localhost4 || a.ClientIP == global.Localhost6 || a.ClientIP == "") {
		...
		level.Info(log.Logger).Log(global.LogKeyGUID, a.GUID, global.FeatureLua, "localhost")
	}
...
func (a *AuthState) featureTLSEncryption() (triggered bool) {
	...
	if !(a.ClientIP == global.Localhost4 || a.ClientIP == global.Localhost6 || a.ClientIP == "") {
		...
		level.Info(log.Logger).Log(global.LogKeyGUID, a.GUID, global.FeatureTLSEncryption, "localhost")
	}
...
func (a *AuthState) featureRelayDomains() (triggered bool) {
	...
	if !(a.ClientIP == global.Localhost4 || a.ClientIP == global.Localhost6 || a.ClientIP == "") {
		...
		level.Info(log.Logger).Log(global.LogKeyGUID, a.GUID, global.FeatureRelayDomains, "localhost")
	}
...
func (a *AuthState) featureRBLs(ctx *gin.Context) (triggered bool, err error) {
	...
	if !(a.ClientIP == global.Localhost4 || a.ClientIP == global.Localhost6 || a.ClientIP == "") {
				...
				go func(rbl config.RBL, rblChan chan int) {
					...
					if errRBL != nil {
						if strings.HasSuffix(errRBL.Error(), "no such host") {
							...
							level.Error(log.Logger).Log(global.LogKeyGUID, a.GUID, global.LogKeyError, errRBL)
						}
		...
		level.Info(log.Logger).Log(global.LogKeyGUID, a.GUID, global.FeatureRBL, "localhost")
	}
