...
| `saml-slo-missing-payload-rejected`                   | SAML SLO negative             | An attacker probes `/saml/slo` without either `SAMLRequest` or `SAMLResponse`.                                                                                | Protocol admission rejects the malformed payload with HTTP 400 before canonical state handling.                                                                                      |
| `saml-slo-ambiguous-payload-rejected`                 | SAML SLO negative             | Payload parameter pollution sends both `SAMLRequest` and `SAMLResponse` in one SLO message.                                                                  | Protocol admission rejects the ambiguous payload with HTTP 400 before canonical state handling.                                                                                      |
| `saml-slo-duplicate-request-rejected`                 | SAML SLO negative             | Duplicate `SAMLRequest` parameters attempt to exploit parser disagreement.                                                                                    | Protocol admission rejects the duplicated parameter with HTTP 400 before canonical state handling.                                                                                   |
