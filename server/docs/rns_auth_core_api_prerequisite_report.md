...
| Exact legacy metric compatibility | Complete | A signer-gated, module-scoped compatibility facade accepts only exact operator allowlists for metric name, type, help, labels, and buckets. It safely reuses an already registered Lua collector only when the complete contract is identical; drift fails closed. It does not expose a Prometheus registerer. Native metrics remain supplemental, with shared-observation and double-registration coverage. |
