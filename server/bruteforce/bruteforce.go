...
func (bm *bucketManagerImpl) CheckRepeatingBruteForcer(rules []config.BruteForceRule, network **net.IPNet, message *string) (withError bool, alreadyTriggered bool, ruleNumber int) {
	...
	*network = nil
	...
	if len(candidates) > 0 {
		...
		rCtx, rSpan := tr.StartClient(dCtx, "redis.hmget_bruteforce_preresult",
			attribute.Int("fields.count", len(fields)),
		)
		vals, errHM := rediscli.GetClient().GetReadHandle().HMGet(rCtx, key, fields...).Result()
		rSpan.End()
		cancel()
