...
| Mixed Lua/native subject ordering | Complete | A policy `after:` edge from `lua.subject` to `plugin.subject` creates one deterministic native boundary: non-deferred Lua subjects, native subject bridge, then deferred Lua subjects. A real `AuthState.SubjectLua` host-path test proves `geoip_history -> native_rns_ldap -> director_routing`; legacy configurations without a cross-source edge retain the previous whole-Lua-before-native order. A second native boundary fails closed at compile time. |
