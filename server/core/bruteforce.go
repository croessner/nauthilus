...
func (a *AuthState) getPasswordHistoryRedisHashKey(withUsername bool) (key string) {
	if withUsername {
		...
		key = config.GetFile().Server.Redis.Prefix + definitions.RedisPwHashKey + fmt.Sprintf(":%s:%s", accountName, a.ClientIP)
	} else {
		key = config.GetFile().Server.Redis.Prefix + definitions.RedisPwHashKey + ":" + a.ClientIP
	}
...
func (a *AuthState) getBruteForceBucketRedisKey(rule *config.BruteForceRule) (key string) {
	...
	key = config.GetFile().Server.Redis.Prefix + "bf:" + fmt.Sprintf(
...
func (a *AuthState) checkTooManyPasswordHashes(key string) bool {
	...
	if length, err := rediscli.GetClient().GetReadHandle().HLen(a.HTTPClientContext, key).Result(); err != nil {
		...
		if length > int64(config.GetFile().Server.MaxPasswordHistoryEntries) {
			return true
		}
	}
...
func (a *AuthState) saveFailedPasswordCounterInRedis() {
	...
	for index := range keys {
		...
		if err := rediscli.GetClient().GetWriteHandle().Expire(a.HTTPClientContext, keys[index], config.GetFile().Server.Redis.NegCacheTTL).Err(); err != nil {
			level.Error(log.Logger).Log(definitions.LogKeyGUID, a.GUID, definitions.LogKeyMsg, err)
		}
...
func (a *AuthState) setPreResultBruteForceRedis(rule *config.BruteForceRule) {
	key := config.GetFile().Server.Redis.Prefix + definitions.RedisBruteForceHashKey
...
func (a *AuthState) processPWHist() (accountName string) {
	...
	if err = rediscli.GetClient().GetWriteHandle().Expire(a.HTTPClientContext, key, config.GetFile().Server.Redis.NegCacheTTL).Err(); err != nil {
		level.Error(log.Logger).Log(definitions.LogKeyGUID, a.GUID, definitions.LogKeyMsg, err)
	}
...
func (a *AuthState) updateAffectedAccount() {
	...
	key := config.GetFile().Server.Redis.Prefix + definitions.RedisAffectedAccountsKey
...
func getPWHistIPsRedisKey(accountName string) string {
	key := config.GetFile().Server.Redis.Prefix + definitions.RedisPWHistIPsKey + ":" + accountName
...
func (a *AuthState) CheckBruteForce() (blockClientIP bool) {
	...
	if config.GetFile().BruteForce.HasSoftWhitelist() {
		if util.IsSoftWhitelisted(a.Username, a.ClientIP, *a.GUID, config.GetFile().BruteForce.SoftWhitelist) {
			a.AdditionalLogs = append(a.AdditionalLogs, definitions.LogKeyBruteForce)
	...
	if len(config.GetFile().BruteForce.IPWhitelist) > 0 {
		if a.IsInNetwork(config.GetFile().BruteForce.IPWhitelist) {
			a.AdditionalLogs = append(a.AdditionalLogs, definitions.LogKeyBruteForce)
	...
	for _, bruteForceService := range config.GetFile().Server.BruteForceProtocols {
