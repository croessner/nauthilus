...
import (
	...
	coreidp "github.com/croessner/nauthilus/v3/server/idp"
...
func TestCanIntrospectAccessToken(t *testing.T) {
	cases := []struct {
		...
		{name: "policy denied", audience: "nauthilus:policy", clientID: "caller", tokenType: definitions.TokenTypeAccessToken, allowed: true},
		{name: "mixed resources", audience: []string{definitions.AudienceBackchannelAPI, "nauthilus:policy"}, clientID: "caller", tokenType: definitions.TokenTypeAccessToken, allowed: true},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			client := &config.OIDCClient{ClientID: "inspector", ClientSecret: secret.New("test-secret"), AllowBackchannelIntrospection: tc.allowed}
...
func TestOIDCHandler_OpaqueBackchannelIntrospection(t *testing.T) {
	...
	session := &coreidp.OIDCSession{
		ClientID: "service-caller", UserID: "service-caller", ServiceToken: true,
		Scopes: []string{definitions.ScopeAuthenticate}, AccessTokenAudience: definitions.AudienceBackchannelAPI,
		AccessTokenIssuer: f.issuer, AccessTokenExpiresAt: time.Now().Add(time.Hour),
	}
	...
	key := "test:oidc:access_token:" + token
