...
func RouterGroup(cfg config.File, logger *slog.Logger, langManager corelang.Manager, path string, router gin.IRouter, getHandler gin.HandlerFunc, postHandler gin.HandlerFunc) *gin.RouterGroup {
	group := router.Group(path, CreateMiddlewareChain(cfg, logger, langManager)...)
