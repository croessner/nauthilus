...
activation. Authentication learning uses the separately registered synchronous obligation
...
Select `authn/plugin.reputation.learn_outcome` as a host-synchronous obligation only on
...
Learning failures cause Policy Tempfail before the response is sent. Kafka
acceptance requires a broker acknowledgement; there is no filesystem outbox. The removed Lua writer, registry facts, ambient tuning and analytics
