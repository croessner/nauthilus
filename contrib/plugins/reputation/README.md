...
activation. Authentication learning uses the separately registered capture-only obligation
...
`reputation/learn_outcome/obligation/execute/authn/authenticate`; it has no API
...
Select `authn/plugin.reputation.learn_outcome` as a capture-only obligation only on
...
Learning capture never vetoes the authentication result. A dedicated bounded queue
and fixed worker pool perform Redis admission and Kafka delivery after capture;
queue overflow and terminal delivery failure are observable learning losses. Kafka
acceptance requires a broker acknowledgement; local buffering is volatile and there
is no filesystem outbox. See [queue settings, retries, shutdown and metrics](../../../server/docs/reputation_journal.md#authentication-queue-sizing-and-telemetry). The removed Lua writer, registry facts, ambient tuning and analytics
...
`partial`, `rejected`, `quota_exceeded`, `unavailable`, `skipped`, `queued`,
`buffered`, `queue_full`, `expired`, `shutdown`, `worker_panic` or `retried`. No event identity, subject,
...
`partial` even when a quota stops the remaining work. External observation quota failures are still returned to their caller. Background
authentication learning records quota failures without changing login decisions;
neither path acknowledges them as successful learning.
