...
func listBlockedAccounts(ctx context.Context, filterCmd *FilterCmd, guid string) (*BlockedAccounts, error) {
	...
	if err != nil {
		...
		for _, account := range accounts {
			...
			key = config.GetFile().Server.Redis.Prefix + definitions.RedisPWHistIPsKey + ":" + account
...
func prepareRedisUserKeys(ctx context.Context, guid string, accountName string) ([]string, config.StringSet) {
	...
	userKeys.Set(config.GetFile().Server.Redis.Prefix + definitions.RedisUserPositiveCachePrefix + "__default__:" + accountName)

	if ips != nil {
		for _, ip := range ips {
			userKeys.Set(config.GetFile().Server.Redis.Prefix + definitions.RedisPwHashKey + ":" + accountName + ":" + ip)
			userKeys.Set(config.GetFile().Server.Redis.Prefix + definitions.RedisPwHashKey + ":" + ip)
		}
	}
...
func removeUserFromCache(ctx context.Context, userCmd *FlushUserCmd, userKeys config.StringSet, guid string, removeHash bool) []string {
	...
	redisKey := config.GetFile().Server.Redis.Prefix + definitions.RedisUserHashKey
