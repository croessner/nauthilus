...
-- Derives an environment signal from ntc:top_failed_logins (count/rank by username)
-- This environment source is read-only against Redis and can be enabled safely. It enriches
...
local function maybe_snapshot_topN(client, now, request)
    ...
    local gate_key = nauthilus_util.get_redis_key(request, "environment:" .. N .. ":last_snapshot")
...
function nauthilus_call_environment(request)
    ...
    if nauthilus_util.is_table(rt) then
        ...
        if is_hot then
            rt.environment_failed_login_hotspot = true
