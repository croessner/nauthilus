...
// validateIDPOIDCIntrospectionSettings limits delegated introspection to authenticated static clients.
func (f *FileSettings) validateIDPOIDCIntrospectionSettings() error {
	...
	for idx := range f.IDP.OIDC.Clients {
		...
		if client.AllowBackchannelIntrospection && !client.AllowsBackchannelIntrospection() {
			return NewValidationProblem(fmt.Sprintf("identity.oidc.clients[%d].allow_backchannel_introspection", idx),
				"backchannel introspection requires a static client with confidential client authentication")
		}
	}
