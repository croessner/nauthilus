...
FROM --platform=$TARGETPLATFORM golang:1.27.0-alpine3.24 AS builder
...
RUN GIT_TAG=$(git describe --tags --abbrev=0) && echo "tag="${GIT_TAG}"" && \
    ...
    -trimpath \
    -ldflags="-s -w -X main.buildTime=$(date -u +'%Y-%m-%dT%H:%M:%SZ') -X main.version=${GIT_TAG}-${GIT_COMMIT} -X github.com/croessner/nauthilus/v3/server/config.nauthilusConfDir=${NAUTHILUS_CONF_DIR} -X github.com/croessner/nauthilus/v3/server/config.nauthilusPluginsDir=${NAUTHILUS_PLUGINS_DIR}" \
    -o nauthilus . && \
