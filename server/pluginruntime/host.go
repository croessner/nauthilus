...
import (
	...
	"github.com/croessner/nauthilus/v4/server/lualib/smtp"
...
// goWithLogger starts one detached worker bounded by the host service lifetime.
func (h *Host) goWithLogger(ctx context.Context, _ string, logger pluginapi.Logger, fn func(context.Context) error) {
	...
	h.workers.Go(func() {
		...
		runPluginWorker(workerCtx, logger, fn)
	})
}

// runPluginWorker contains worker panics and reports bounded failures.
func runPluginWorker(ctx context.Context, logger pluginapi.Logger, fn func(context.Context) error) {
	defer func() {
