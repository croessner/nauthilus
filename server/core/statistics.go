...
func LoadStatsFromRedis(ctx context.Context) {
	...
	redisLoginsCounterKey := config.GetFile().GetServer().Redis.Prefix + definitions.RedisMetricsCounterHashKey + "_" + strings.ToUpper(config.GetFile().GetServer().GetInstanceName())
